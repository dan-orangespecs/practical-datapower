...
<!-- 
	...
	This stylesheet is part of an AAA action in DataPower. The goal is to 
	authorize an incoming request by inspecting the client certificate, looking 
	up an XML configuration file and determining if there is a match. 
	
	When a match is made, the stylesheet outputs <accepted/>
	
	When a error occurs, the stylesheer outputs the reason. 
